...
void ALevel_Navmesh::Tick(float DeltaTime)
{
	...
	// Todo: Draw the portals travelled through with SSFA
	 if (bDrawPortals)
	 {
		...
		DrawDebugLine(
			GetWorld(),
			FVector{ portal.P1, 15.f },
			FVector{ portal.P2, 15.f },
			FColor::Cyan,
			false,
			10.f,
			0,
		...
		DrawDebugPoint(GetWorld(), FVector{ portal.P1, 15.f }, 8.f, FColor::Red, false, 10.f);   // right
		DrawDebugPoint(GetWorld(), FVector{ portal.P2, 15.f }, 8.f, FColor::Green, false, 10.f); // left
		}
	 }
...
void ALevel_Navmesh::SetTarget()
{
	...
	DebugDrawPath = Path;
	debugPortals = Pathfinder.GetPortals();
